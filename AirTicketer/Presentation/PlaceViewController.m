...
@interface PlaceViewController () <UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating>
...
- (NSArray *)sourceArray {
    if (_searchController.isActive && _searchController.searchBar.text) {
        return _searchArray;
    } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_segmentedControl.selectedSegmentIndex == 0) {
        City *city = [self.sourceArray objectAtIndex:indexPath.row];
        cell.textLabel.text = city.name;
        cell.detailTextLabel.text = city.code;
    }
    else if (_segmentedControl.selectedSegmentIndex == 1) {
        Airport *airport = [self.sourceArray objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.delegate selectPlace:[self.sourceArray objectAtIndex:indexPath.row] withType:_placeType andDataType:dataType];
